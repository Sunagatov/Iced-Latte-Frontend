...
export async function mergeCarts(cartItemIds: ICartPushItems): Promise<ICart> {
  const response: AxiosResponse<ICart> = await api.post('/cart/items', cartItemIds)
...
export async function removeCartItem(ids: string[]): Promise<ICart> {
  ...
  const response: AxiosResponse<ICart> = await api.delete('/cart/items', { data: deleteItems })
  return response.data
}

export async function changeCartItemQuantity(item: ICartUpdatedItem): Promise<ICart> {
