...
const AIReviewSummary = ({ summary, reviewsCount }: AIReviewSummaryProps) => {
  ...
  const displayText =
    isLong && !expanded ? summary.slice(0, 180).trimEnd() + '…' : summary
  ...
  return (
    <div className="flex h-full flex-col rounded-2xl border border-black/6 bg-white p-5 shadow-sm sm:p-6">
      ...
      <div className="relative pl-5">
        <span className="absolute top-[-4px] left-0 text-3xl leading-none text-brand/30 select-none">
          &ldquo;
        </span>
        <p className="text-sm leading-relaxed text-slate-700 sm:text-[15px]">
          ...
          {isLong && (
            <button
              ...
              className="ml-1.5 font-semibold text-brand hover:underline focus:outline-none"
            >
              {expanded ? 'less' : 'more'}
            </button>
          )}
        </p>
        <span className="text-3xl leading-none text-brand/30 select-none">
          &rdquo;
        </span>
      </div>
