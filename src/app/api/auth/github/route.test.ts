...
function makeRequest(next?: string): NextRequest {
  const url = next
    ? `http://localhost/api/auth/github?next=${encodeURIComponent(next)}`
...
describe('github auth route', () => {
  ...
  it('allows safe next values with query strings', async () => {
    const res = await GET(makeRequest('/checkout?coupon=SAVE10'))
    const location = new URL(res.headers.get('location')!)

    expect(res.status).toBe(307)
    expect(location.toString()).toContain('/auth/oauth/github')
    expect(location.searchParams.get('redirectUrl')).toBe(
      'https://frontend.example/auth/github/callback?next=%2Fcheckout%3Fcoupon%3DSAVE10',
    )
  })

  it('redirects the browser to backend oauth initiation', async () => {
    const res = await GET(makeRequest('/orders'))
    const location = new URL(res.headers.get('location')!)

    expect(res.status).toBe(307)
    expect(location.toString()).toContain('/auth/oauth/github')
    expect(location.searchParams.get('redirectUrl')).toBe(
      'https://frontend.example/auth/github/callback?next=%2Forders',
    )
  })
  ...
  it('handles missing next', async () => {
    ...
    expect(res.status).toBe(307)
    expect(location.searchParams.get('redirectUrl')).toBe(
      'https://frontend.example/auth/github/callback',
    )
  })
  ...
  it('falls back to the request origin when frontend URL is not configured', async () => {
    ...
    expect(res.status).toBe(307)
    expect(redirectUrl).toBe('http://localhost/auth/github/callback?next=%2Forders')
  })
  ...
  it('builds the backend OAuth URL when the API URL has a trailing slash', async () => {
    ...
    expect(res.status).toBe(307)
    expect(location.toString()).toContain('http://backend/api/v1/auth/oauth/github')
  })
