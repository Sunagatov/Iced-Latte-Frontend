...
describe('GoogleCallbackPage', () => {
  ...
  it('exchanges the handoff code once under React Strict Mode', async () => {
    ...
    window.history.replaceState(null, '', `/auth/google/callback#oauthCode=${oauthCode}`)
  ...
  it('redirects invalid handoff codes to sign in', async () => {
    window.history.replaceState(null, '', '/auth/google/callback#oauthCode=bad')

    render(<GoogleCallbackPage />)

    await waitFor(() => {
      expect(replace).toHaveBeenCalledWith('/signin?error=google_auth_failed')
    })
  ...
  it('preserves safe next when the provider returns an error', async () => {
    ...
    await waitFor(() => {
      expect(replace).toHaveBeenCalledWith(
        '/signin?error=google_auth_failed&next=%2Forders',
      )
    })
  ...
  it('preserves safe next when the handoff exchange fails', async () => {
    ...
    await waitFor(() => {
      expect(replace).toHaveBeenCalledWith(
        '/signin?error=google_auth_failed&next=%2Fcheckout%3Fcoupon%3DSAVE10',
      )
    })
