...
async function setup(page: Page, { orderStatus = 200 }: { orderStatus?: number } = {}) {
...
test('cart is cleared after successful order — cart-count badge gone', async ({ page }) => {
  ...
  await page.route('**/api/proxy/**', async (route) => {
    ...
    else if (url.includes('/addresses')) await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify([]) })
