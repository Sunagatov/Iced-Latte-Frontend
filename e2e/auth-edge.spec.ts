...
test('forgot password form submits email and shows confirmation', async ({ page }) => {
  ...
  await Promise.race([
    expect(page.locator('input[type="email"]')).not.toBeVisible({ timeout: 8000 }),
    expect(page.locator('body')).toContainText(/sent|check|email/i, { timeout: 8000 }),
  ]).catch(() => {
...
test('sign-in form blocks submit with empty required fields', async ({ page }) => {
...
test('sign-up form shows validation error for weak password', async ({ page }) => {
