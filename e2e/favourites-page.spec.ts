...
test('removing a favourite updates the list', async ({ page }) => {
  ...
  await page.goto('http://localhost:3000')
  await page.evaluate((t) => localStorage.setItem('token', JSON.stringify({ state: { token: t, refreshToken: null, isLoggedIn: true }, version: 0 })), FAKE_TOKEN)
  // Seed fav store with the product
  await page.evaluate((id) => {
    localStorage.setItem('fav-storage', JSON.stringify({ state: { favouriteIds: [id], favourites: [{ id, name: 'Test Coffee', price: 9.99, productFileUrl: null, brandName: 'Brand', sellerName: 'Seller', averageRating: 4.5, reviewsCount: 1, quantity: 250, description: 'desc', active: true }] }, version: 0 }))
  }, PRODUCT_ID)
  await page.goto('/favourites')
  await page.waitForSelector('[data-testid="fav-element"]', { timeout: 8000 })
  // Click the heart button to remove
