 ...
 *
 * Usage in real-mode tests:
 *   await seedCart(page, [{ productId: '...', productQuantity: 1 }])
 *   await seedFavourites(page, ['product-id-1'])
 *   await clearCart(page)
 *   await removeFavourite(page, 'product-id-1')
 */
...
export async function clearCart(page: Page): Promise<void> {
  if (!IS_REAL) return
  // DELETE /cart/items requires shoppingCartItemIds — fetch cart first to get them
