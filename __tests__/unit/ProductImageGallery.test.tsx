...
jest.mock('@/shared/utils/getImgUrl', () => ({ __esModule: true, default: (url: string | null, fallback: string) => url ?? fallback }))

describe('ProductImageGallery', () => {
  it('renders single image without navigation', () => {
    render(<ProductImageGallery productFileUrl="/img.jpg" productName="Coffee" />)
    expect(screen.getByAltText('Coffee')).toBeInTheDocument()
    expect(screen.queryByLabelText('Previous image')).not.toBeInTheDocument()
  })

  it('renders thumbnails and arrows for multiple images', () => {
    render(<ProductImageGallery productFileUrl={null} productImageUrls={['/a.jpg', '/b.jpg']} productName="Latte" />)
  ...
  it('wraps around to last image on prev from first', () => {
    render(<ProductImageGallery productFileUrl={null} productImageUrls={['/a.jpg', '/b.jpg', '/c.jpg']} productName="Latte" />)
